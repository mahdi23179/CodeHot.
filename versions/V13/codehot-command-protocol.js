...
(function (global) {
  ...
  // ── V13 physics commands (BUG-002) ─────────────────────────────
  // Deterministic runtime scripts over the documented API: ground = object's
  // start Y, gravity 22 u/s², jump impulse 9, speed default 4 u/s. All steps
  // multiply (ctx.delta || 0.016) so they advance even when frame timing is
  // unavailable in the host.
  function physScript(kind, params) {
    var key = String(params.KEY || params.JUMP_KEY || 'Space').replace(/[^A-Za-z0-9]/g, '') || 'Space';
    var speed = Number(params.SPEED);
    if (!isFinite(speed) || speed <= 0) speed = 4;
    var head =
      'start(ctx, self) {\n' +
      '  self.__ph = { groundY: Math.min(self.position.y, 0), vy: 0, onGround: true, keys: {} };\n';
    var keyListen =
      '  self.__ph.kd = function (e) { self.__ph.keys[e.code] = true; };\n' +
      '  self.__ph.ku = function (e) { self.__ph.keys[e.code] = false; };\n' +
      '  document.addEventListener("keydown", self.__ph.kd);\n' +
      '  document.addEventListener("keyup", self.__ph.ku);\n';
    var updateOpen =
      '}\n' +
      'update(ctx, self) {\n' +
      '  var d = (ctx.delta || 0.016);\n' +
      '  self.__ph = self.__ph || { groundY: Math.min(self.position.y, 0), vy: 0, onGround: true, keys: {} };\n' +
      '  var p = self.position;\n';
    var gravity =
      '  self.__ph.vy -= 22 * d;\n' +
      '  p.y += self.__ph.vy * d;\n' +
      '  if (p.y <= self.__ph.groundY) { p.y = self.__ph.groundY; self.__ph.vy = 0; self.__ph.onGround = true; }\n';
    var moveKeys =
      '  var k = self.__ph.keys || {};\n' +
      '  if (k.ArrowLeft || k.KeyA) p.x -= ' + speed + ' * d;\n' +
      '  if (k.ArrowRight || k.KeyD) p.x += ' + speed + ' * d;\n' +
      '  if (k.ArrowUp || k.KeyW) p.z -= ' + speed + ' * d;\n' +
      '  if (k.ArrowDown || k.KeyS) p.z += ' + speed + ' * d;\n';
    var jumpStmt =
      '  if (k["' + key + '"] && self.__ph.onGround) { self.__ph.vy = 9; self.__ph.onGround = false; }\n';
    var tail =
      '}\n' +
      'stop(ctx, self) {\n' +
      '  if (self.__ph) {\n' +
      '    if (self.__ph.kd) document.removeEventListener("keydown", self.__ph.kd);\n' +
      '    if (self.__ph.ku) document.removeEventListener("keyup", self.__ph.ku);\n' +
      '    if (self.__ph.jumpFn) document.removeEventListener("keydown", self.__ph.jumpFn);\n' +
      '  }\n' +
      '}';
    if (kind === 'GRAVITY') {
      return head + updateOpen + gravity + tail;
    }
    if (kind === 'JUMP_KEY') {
      return head +
        '  self.__ph.jumpFn = function (e) {\n' +
        '    if (e.code === "' + key + '" && !e.repeat && self.__ph.onGround) { self.__ph.vy = 9; self.__ph.onGround = false; }\n' +
        '  };\n' +
        '  document.addEventListener("keydown", self.__ph.jumpFn);\n' +
        updateOpen + gravity + tail;
    }
    if (kind === 'MOVE_ON_KEYS') {
      return head + keyListen + updateOpen + moveKeys + tail;
    }
    // PHYSICS_SETUP: gravity + jump + key movement in one script.
    return head + keyListen + updateOpen + moveKeys + jumpStmt + gravity + tail;
  }
  Registry.register({
    name: 'GRAVITY', undoable: false, target: 'object',
    params: {},
    executor(params) {
      var t = requireTarget(params); if (t.error) return t.error;
      var err = validateAndStore(t.id, physScript('GRAVITY', params), t.object.name);
      if (err) return err;
      return { ok: true, message: 'Gravity attached to ' + t.object.name + ' / گرانش به ' + t.object.name + ' متصل شد.' };
    }
  });
  Registry.register({
    name: 'JUMP_KEY', undoable: false, target: 'object',
    params: { KEY: { type: 'string' } },
    executor(params) {
      var t = requireTarget(params); if (t.error) return t.error;
      var err = validateAndStore(t.id, physScript('JUMP_KEY', params), t.object.name);
      if (err) return err;
      return { ok: true, message: 'Jump (Space) attached to ' + t.object.name };
    }
  });
  Registry.register({
    name: 'MOVE_ON_KEYS', undoable: false, target: 'object',
    params: { SPEED: { type: 'number' } },
    executor(params) {
      var t = requireTarget(params); if (t.error) return t.error;
      var err = validateAndStore(t.id, physScript('MOVE_ON_KEYS', params), t.object.name);
      if (err) return err;
      return { ok: true, message: 'Keyboard movement attached to ' + t.object.name };
    }
  });
  Registry.register({
    name: 'PHYSICS_SETUP', undoable: false, target: 'object',
    params: { SPEED: { type: 'number' }, KEY: { type: 'string' } },
    executor(params) {
      var t = requireTarget(params); if (t.error) return t.error;
      var err = validateAndStore(t.id, physScript('SETUP', params), t.object.name);
      if (err) return err;
      return { ok: true, message: 'Physics (gravity+jump+keys) attached to ' + t.object.name };
    }
  });
  Registry.register({
    name: 'CAMERA_FOLLOW', undoable: false, target: 'object',
    params: { OFFSET_Y: { type: 'number' }, OFFSET_Z: { type: 'number' } },
    executor(params) {
      var t = requireTarget(params); if (t.error) return t.error;
      var oy = Number(params.OFFSET_Y); if (!isFinite(oy) || oy === 0) oy = 4;
      var oz = Number(params.OFFSET_Z); if (!isFinite(oz) || oz === 0) oz = 7;
      var quoted = String(t.object.name || '').replace(/\\/g, '\\\\').replace(/"/g, '\\"');
      var code =
        'start(ctx, self) {}\n' +
        'update(ctx, self) {\n' +
        '  var tgt = CodeHot.getObjectByName("' + quoted + '");\n' +
        '  if (!tgt || !tgt.position) return;\n' +
        '  self.setPosition(tgt.position.x, tgt.position.y + ' + oy + ', tgt.position.z + ' + oz + ');\n' +
        '  self.lookAt(tgt.position.x, tgt.position.y, tgt.position.z);\n' +
        '}\n' +
        'stop(ctx, self) {}';
      var err = validateAndStore('__codehot_camera__', code, 'Camera');
      if (err) return err;
      return { ok: true, message: 'Camera will follow ' + t.object.name + ' during Play / دوربین موقع Play دنبال ' + t.object.name + ' می‌آید.' };
    }
  });
  Registry.register({
